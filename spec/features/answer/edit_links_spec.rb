...
feature 'User can edit, add, delete links of his answer', %q{
  In edit links in my answer
  ...
  given!(:answer) { create(:answer, question: question, links_attributes: [link_google, link_dzen], author: user) }
  ...
  scenario 'User edit link when edit answer', js: true do
    within all(".edit_form .nested-fields").first do
  ...
  scenario 'User add new links when edit answer', js: true do
    within '.edit_form' do
      click_on '+ Add link'
  ...
  scenario 'User remove link when edit answer', js: true do
    expect(page).to have_link 'Google link', href: url_google
    expect(page).to have_link 'Dzen link', href: url_dzen
